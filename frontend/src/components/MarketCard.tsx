...
export default function MarketCard({ market }: { market: Market }) {
  ...
  return (
    <Link
      ...
      className="group relative block min-h-[270px] overflow-hidden rounded-lg border border-emerald-900/40 bg-[#092016] p-6 shadow-[0_18px_50px_rgba(0,0,0,0.2)] transition hover:-translate-y-1 hover:border-emerald-400/80"
    >
      ...
      <div className="mb-3 text-sm text-emerald-100/45">
        {market.company || 'JSE listed'} · Closes {new Date(market.endDate).toLocaleDateString('en-ZA', { month: 'short', year: 'numeric' })}
      </div>
      <h3 className="text-lg font-semibold leading-snug text-white mb-5">{market.title}</h3>
      ...
      <div className="absolute inset-0 translate-y-full bg-[#06120d]/98 p-6 opacity-0 transition duration-200 group-hover:translate-y-0 group-hover:opacity-100 group-focus-visible:translate-y-0 group-focus-visible:opacity-100">
